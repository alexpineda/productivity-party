...
const { PRODUCTIVITY_SCORE_UPDATE_INTERVAL } = require('../config');
...
console.log(`🔄 Will fetch ${API_URL} every ${PRODUCTIVITY_SCORE_UPDATE_INTERVAL} minute(s)`);
  ...
  const req = http.get(API_URL, (res) => {
    ...
    res.on('end', () => {
      try {
        ...
        if (response.success) {
          console.log(`✅ Successfully processed ${response.processedBlockCount} blocks`);
